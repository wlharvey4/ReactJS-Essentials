...
our build process.  Create a small example ~gulpfile.js~ in your
~<dir>/snapterest/~ directory and add the following content to it[fn::from
...
=default=.  You now have a working Gulp build system.  Now create a default
task that will package your source and dependency modules using [[Browserify][Browserify]].
