...
- A string could be an HTML tg name such as `div', `p', `h1', and so on.  React
  supports all the common HTML tags and attributes.  For a complete list of
  HTML tags and attributes supported by React, refer to:
...
#+NAME:first-stateless-component
...
var ReactClass = React.createClass({			(ref:1 class)
    render: function () {
        return React.createElement('h1',			(ref:2 element)
                                   { className: 'header' },
                                   'React Component');
    }
});

var reactComponentElement = React.createElement(ReactClass); (ref:3 component)
...
2. Creating a React component element. [[(2 element)]]
3. Creating a React component. [[(3 component)]]

{{{heading(Create a React Component)}}}
...
1. Create a ~ReactClass~ by calling the ~React.createClass()~ function and
   ...
   #+CINDEX:@code{React.createElement()} function
   #+CINDEX:~ReactClass~ as @samp{type} parameterx
2. Create a ~ReactComponentElement~ by calling the ~React.createElement()~
   function and providing our ~ReactClass~ as its =type= parameter.  In Chapter
   2, we learned that the =type= parameter can be either a string or a
   ~ReactClass~.  In this chapter, you'll learn more about the latter.

   #+CINDEX:@code{ReactComponent}
   #+CINDEX:@code{ReactDOM.render()} function
3. Create a ~ReactComponent~ by calling the ~ReactDOM.render()~ function and
...
{{{heading(Specification Object)}}}

#+CINDEX:state, specification object
#+CINDEX:rendered, component by specification object
#+CINDEX:@code{render()} method
The /specification object/ 1) encapsulates a component's state and 2) describes
how a component is rendered.  At the very minimum, the React component needs to
have a ~render()~ method so that it returns at least =null= or =false=.  Here
is an example of a specification object in its simplest form:
...
The ~render()~ function is responsible for telling React how to render your
...
This example shows how we can encapsulate our Reacxt element inside our React
component.  We create a ~ReactElement~ of type =h1= with the properties object
...
the benefit of having a ~render()~ function if we can render the exact same
markup without it?

The advantage of having a ~render()~ function is that, was with any other
function, before it returns a value, it can choose what value to return.  So
far, you've seen two examples of the ~render()~ method: one that returns =null=
and one that returns a React element.  We can merge the two and add a condition
...
{{{heading(Passing Data to a React Component)}}}
...
#+CINDEX:attributes to HML elements
We learned that the ~React.createElement()~ method accepts the =props=
parameter.  We used it to pass attributes to our HTML elements, but we didn't
discuss what happens behind the scenes and why attributes passed to the =props=
object get rendered.

#+CINDEX:@code{this.props}
#+CINDEX:data with @samp{props} parameter
Any data that you put in the =props= object and pass to the
~React.createElement()~ function can be accessed inside the ~render()~ method
of ~ReactComponent~ via ~this.props~.  Once you have accessed data from
...
In this example, we're using ~this.props~ insider our ~render()~ method to
access the =header= property.  We're then passing ~this.props.header~ directly
to the ~React.createElement()~ method as a child string element.

In the preceding example, we can pass the value of ~isHidden~ as another
property of the ~this.state~ object:
...
All the components are stateful and they can mnanage their own state.
...
The idea is to separate your components into two concerns: how to handle the
user interface interaction logic and how to render data.
...
#+CINDEX:manage state
Statefule components arre the most appropriate place for you application to
...
return.  Let's add this function to our React component:
...
#+NAME:stateful-component-reacts
...
The first thing wedo is set our initial state object by returning it from the
...
React Component=.  Now we can access this state4 object in our ~render()~
method via ~this.state~.  Inside our ~render()~ method, we create three React
...
choose depends on the valuer of ~this.state.isHeaderHidden~.  The current state
of our component directly affects what the ~render()~ function will render.
...
function that itself is a property of the component object.  For example, we
...
~createElement()~ method (see [[(createElement)]]).
...
Notice that passing the ~onClick~ property to our ~createElement()~ function in
...
single event listenerm, and delegates them to their appropriate event handlers.
...
event handler to the ~click~ event that updates the value of the
~isHeaderHidden~ property.  But have you noticed that the user interaction does
not update the value of another property that we store in our state, ~titler~?
...
a ~state~ object, and compute the rest of the data based on whast's in ~state~
