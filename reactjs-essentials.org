...
version of Node.js as of {{{date}}} is src_sh[:eval query :exports
results]{node -v}.] because, at the time of writing this book, that's the
latest version of Node.js that Jest supports.[fn::The currently available
versions of Node.js as of the publication of this document ({{{date}}}) are
=10.15.3 LTS= and =11.11.0 Current=.  The current version of Jest is =24.1=,
which appears to work with the current version of Node.js and React.]
...
Node.js, so it was installed by the previous step.  To test:
     : $ npm -v
     : 1.4.28
     [fn::Currently installed npm version as of {{{date}}} is src_sh[:eval
     query :exports results]{npm -v}]
...
git installed is src_sh[:eval query :exports results]{git --version} using the
...
#+BEGIN_SRC js :tangle snapterest/gulpfile.js
...
#+BEGIN_SRC sh :dir snapterest :results output :exports none
gulp
#+END_SRC

#+RESULTS:
: [12:36:23] Using gulpfile ~/Dev/Programming/WebDev/React/ReactJS-Essentials/snapterest/gulpfile.js
: [12:36:23] Starting 'default'...
: [12:36:23] Finished 'default' after 44 ms
...
#+BEGIN_SRC html :tangle snapterest/build/index.html :mkdirp yes
<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
...
#+BEGIN_SRC js :tangle snapterest/source/app.js :mkdirp yes :noweb yes
...
#+BEGIN_SRC js :tangle snapterest/source/components/Stream.react.js :mkdirp yes :noweb yes
...
#+BEGIN_SRC js -r :tangle snapterest/source/components/Header.react.js :mkdirp yes
var React = require('react');
...
#+BEGIN_SRC js -r :tangle snapterest/source/components/Tweet.react.js :mkdirp yes
var React = require('react');
...
#+BEGIN_SRC js :tangle snapterest/source/components/Button.react.js :mkdirp yes
var React = require('react');
...
#+BEGIN_SRC js :tangle snapterest/source/components/CollectionExportForm.react.js :mkdirp yes
var React = require('react');
...
Now, open ~<dir>/snapterest/build/index.html~ in your web browser.  You will
see new tweets appear.  Click on them to add them to your collection.  Click on
them again to remove individual tweets from the collection.  Click on the
{{{key(Empty collection)}}} button to remove all thee tweets from your
...
#+BEGIN_SRC js :tangle snapterest/source/utils/TweetUtils.js :mkdirp yes
...
#+BEGIN_SRC js :tangle snapterest/source/utils/__tests__/TweetUtil-test.js :mkdirp yes
...
[[shell:cd%20snapterest%20&&%20yarn%20add%20jest-cli%20--dev][Install Latest Jest-CLI]]
...
#+RESULTS: add test script to package.json
#+begin_example
{
  "name": "snapterest",
  "version": "1.0.0",
...
[[shell:(cd snapterest; yarn test;)][yarn test]]
...
#+CINDEX:@file{snapterest/source/utils/} directory
This time, we'll create and test the collection utility module.  Create the
~CollectionUtils.js~ file in the ~<dir>/snapterest/source/utils/~ directory:

#+NAME:CollectionUtils module
#+CAPTION[<CollectionUtils Module>]:The ~CollectionUtils~ Module for Testing
#+BEGIN_SRC js :tangle snapterest/source/utils/CollectionUtils.js :mkdirp yes
...
#+BEGIN_SRC js :tangle snapterest/source/utils/__tests__/CollectionUtils-test.js :mkdirp yes
jest.autoMockOff();
...
#+BEGIN_SRC js :tangle snapterest/source/components/__tests__/Header-test.js :mkdirp yes
...
[[shell:(cd%20snapterest;%20yarn%20add%20react-addons-test-utils%20--dev;)][Install TestUtils]]
...
#+BEGIN_SRC js :tangle snapterest/source/components/__tests__/Button-test.js :mkdirp yes
...
#+RESULTS: add babel preprocessor for jest to package.json
#+begin_example
{
  "name": "snapterest",
  "version": "1.0.0",
...
[[shell:(cd%20snapterest;%20yarn%20add%20babel-jest%20--dev;)][yarn add babel-jest --dev]]
...
#+BEGIN_SRC js :tangle snapterest/source/dispatcher :mkdirp yes
...
#+BEGIN_SRC js :tangle snapterest/source/actions/TweetActionCreators.js :mkdir yes
...
#+BEGIN_SRC js :tangle snapterest/source/stores/TweetStore.js :mkdirp yes
var AppDispatcher = require('../dispatcher/AppDispatcher');
...
#+BEGIN_SRC js :tangle snapterest/source/utils/WebAPIUtils.js :mkdirp yesp
var SnapkiteStreamClient = require('snapkite-stream-client');
...
#+BEGIN_SRC js
var WebAPIUtils = reuqire('./utils/WebAPIUtils');
...
#+NAME:Stream.getInitialState from Flux Store
#+BEGIN_SRC js
getInitialState: function () {
    return {
...
public methods, which are called /getters/.

You can get data from a store, but you can't set data on a store directly just
like that.  STores have no public /setter/ methods.  They are purposely
designed with this limitation in mind so taht when you write your application
with Flux, your data can flow in one direction.  This will benefit you hugely
down the road, when you'll need to maintain your Flux application.
...
because it assuemes that we don't know when =TweetStore= updates its tweet with
...
Now when =TweetStore= changes its data, it will triger our
...
React component.  To to this, add the following ~componebntWillUnmount()~
...
need the ~handleTweetToCollection()~ callback function anymore; therefore, in
this component, we're going to change the ~return~.  Replacde it with:

#+NAME:return from Stream component
...
- ~Stream~: :: This listens to changes in =TweetStore=, updates the state with
               a new tweet, and re-renders.


Can you see dhow we can now scale the number of React components, action
