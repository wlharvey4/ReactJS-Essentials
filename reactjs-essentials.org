...
#+CINDEX:Bootstrap
It defines the layout of our web page using the Bootstrap framework.  if you're
not familiar with Boostrap, I strongly recommend that you visit
https://getbootstrap.com/ and read the documentation.  Learning this framework
will empower you to prototype user interfaces in a fast and easy way.
...
#+NAME:Tweet
#+CAPTION:The ~Tweet~ Component in ~Tweet.react.js~
...
in action by building the most complext components in our application, that is,
...
#+BEGIN_SRC js :tangle snapterest/source/components/TweetList.react.js :mkdirp yes
var React = require('react');
var Tweet = require('./Tweet.react');
...
var =TweetList = React.createClass({
    getListOfTweetIds: function () {		(ref:array-of-ids)
        return Object.keys(this.props.tweets);
    },
...
function.  First, we create a list of ~Tweet~ elements.  The
~this.getListOfTweetIds()~ method returns an [[(array-of-ids)][array of tweet IDs]].  Then, for
each tweet ID in that array, we create a ~Tweet~ component.  For this, we will
call the ~map()~ method on our array of tweet IDs and pass the
~this.getTweetElement()~ method as a callback function.

{{{heading(Handling the Optional ~onImageClick~ Property)}}}
...
{{{heading(Using the ~TweetList~ Component)}}}
...
#+BEGIN_SRC js :tangle snapterest/source/components/CollectionControls.react.js :mkdirp yes
var React = require('react');
var Header = require('./Header.react');
...
var CollectionControls = React.createClass({
    ...
    setCollectionNamle: function (name) {
        this.setState({
            name: name,
            isEditingName: false
        });
        ...
        return (
            <div>
                ...
                <Button
            	    ...
            	    handleClick={this.toggleEditCollection}
            	/>
