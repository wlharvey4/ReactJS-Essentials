...
Clone the Snapkite Engine repository into a directory of your choice[fn::I will
set ~<dir>~ to be ~snapkite~ and clone Snapkite-Engine in it.]:
: cd <dir> (i.e. snapkite/)
...
**** Configure Snapkite Engine

#+CINDEX:Snapkite Engine, configure
#+CINDEX:configure Snapkite Engine
<<configure-Snapkite>>Configure Snapkite Engine.  Assuming you are in the
...
#+BEGIN_SRC sh :dir snapkite/snapkite-engine
grep --silent '"consumerKey": ""' config.json && echo 1 || echo 0
#+END_SRC

#+NAME: add-keys-to-config.json
#+HEADER: :noweb yes :results output :exports both
...
if (<<check-config-keys()>> === 1) {
  ...
  console.log("keys already installed");
}
#+END_SRC
...
<<configure-Snapkite-filters>>Configure them.  In order to do so, you need to
create a configuration file for each Snapkite Filter in JSON format and define
some properties in it.  Each Snapkite Filter comes with an example
...
#+BEGIN_SRC sh :dir snapkite/snapkite-engine :results value :exports code
grep --silent '"filters": \[\]' config.json && echo 1 || echo 0
#+END_SRC

#+NAME:Add filters to Snapkite Engine config
#+HEADER: :var c=check-filters-config()
